...
5. 参考 `docs/project_readme_template.md` 重写新项目的 `README.md`。
6. 按需复制 `configs/database.example.yaml` 为 `configs/database.yaml`。
7. 在 `configs/analysis_config.yaml` 中设置项目名称、时间范围、随机种子和数据源 profile。
8. 在 `notebooks/main_analysis.ipynb` 中基于 `docs/analysis_framework.md` 执行数据加载、质量检查、分析和导出。
9. 将复杂 SQL 放入 `sql/`，复杂 Python 逻辑放入 `src/`。
10. 将关键结果导出到 `reports/`，并在 Notebook 中记录输出路径。
11. 基于 `docs/final_analysis_report_template.md` 输出完整数据分析报告。
12. 交付前按 `docs/08_reproducibility_audit_checklists.md` 完成审计。
...
报告应围绕 `docs/analysis_framework.md` 中确认的业务问题组织，而不是简单罗列图表和结果表。建议参考：
