...
- [ ] 最终报告按业务问题组织，而不是简单罗列结果。
- [ ] 报告结论能追溯到 `docs/analysis_framework.md` 中的业务问题。
- [ ] 业务分析结论有数据支撑。
- [ ] 统计分析说明了方法、假设和局限。
...
- 为了展示技术而使用不必要的复杂模型。
- 只报告模型指标，不解释业务意义。
- 只交付代码、Notebook 或结果图表，不交付完整数据分析报告。
...
2. 检查新增需求是否已同步到分析框架
3. 从头运行 notebooks/main_analysis.ipynb
4. 检查是否有报错
5. 检查 SQL 文件是否存在且有头部说明
6. 检查 Python 脚本是否由 Notebook 调用
7. 检查数据质量结果是否展示
8. 检查图表和结果表是否落地
9. 检查完整数据分析报告是否保存
10. 检查报告结论是否能追溯到业务问题和数据证据
11. 检查是否说明局限性
12. 检查是否泄露敏感信息
```
